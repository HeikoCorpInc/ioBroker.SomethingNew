...
function MiLightV6(options) {
    ...
    options.reconnectTimeout  = parseInt(options.reconnectTimeout, 10) || 100000;
    options.disconnectTimeout = parseInt(options.reconnectTimeout, 10) || 5000;
    options.keepAliveTimeout  = parseInt(options.keepAliveTimeout, 10) || 100000;
    ...
    this.zoneCtlRGBWWFactory = function (zoneID) {
        ...
        return {
            ...
            nightMode: function () {
                return [0x31, 0x00, 0x00, 0x08, 0x04, 0x05, 0x00, 0x00, 0x00, zoneID];
            },
            whiteMode: function () {
                return [0x31, 0x00, 0x00, 0x08, 0x05, 0x64, 0x00, 0x00, 0x00, zoneID];
            },
            ...
            colorTempUp: function () {
                ...
                return [0x31, 0x00, 0x00, 0x08, 0x05, colorTemp, 0x00, 0x00, 0x00, zoneID]
            },
            ...
            mode: function (mode) {
                return [0x31, 0x00, 0x00, 0x08, 0x06, mode, 0x00, 0x00, 0x00, zoneID]
            },
            modeSpeedUp: function () {
                return [0x31, 0x00, 0x00, 0x08, 0x04, 0x03, 0x00, 0x00, 0x00, zoneID]
            },
            modeSpeedDown: function () {
                return [0x31, 0x00, 0x00, 0x08, 0x04, 0x04, 0x00, 0x00, 0x00, zoneID]
            },
            link: function () {
                return [0x3D, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, zoneID];
            },
            unlink: function () {
                return [0x3E, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, zoneID];
            },
    ...
    this.zoneCtlRGBWFactory = function (zoneID) {
        ...
        return {
            ...
            link: function () {
                ...
                return [0x3D, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, zoneID]
            },
            unlink: function () {
                log.log('link not captured');
                return [0x3E, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, zoneID]
            },
            command: function (fnName, arg, cb) {
                if (typeof arg === 'function') {
                    cb = arg;
    ...
    this.zoneCtlRGBFactory = function () {
        var color = 0x7A;
        var zoneID = 0x01;
        ...
        return {
            ...
            brightnessUp: function () {
                brightness = Math.min(brightness + 5, 0x64);
                return [0x31, 0x00, 0x00, 0x05, 0x02, 0x01, 0x00, 0x00, 0x00, zoneID]
            },
            brightnessDown: function () {
                brightness = Math.max(brightness - 5, 0x00);
                return [0x31, 0x00, 0x00, 0x05, 0x02, 0x02, 0x00, 0x00, 0x00, zoneID]
            },
            colorUp: function () {
                color = Math.min(color + 5, 0xFF);
                return [0x31, 0x00, 0x00, 0x05, 0x01, color, color, color, color, zoneID]
            },
            colorDown: function () {
                color = Math.max(color - 5, 0x00);
                return [0x31, 0x00, 0x00, 0x05, 0x01, color, color, color, color, zoneID]
            ...
            effectModeNext: function (mode) {
                return [0x31, 0x00, 0x00, 0x05, 0x02, 0x05, 0x00, 0x00, 0x00, zoneID]
            },
            effectModePrev: function (mode) {
                return [0x31, 0x00, 0x00, 0x05, 0x02, 0x06, 0x00, 0x00, 0x00, zoneID]
            },
            modeSpeedUp: function () {
                return [0x31, 0x00, 0x00, 0x05, 0x02, 0x03, 0x00, 0x00, 0x00, zoneID]
            },
            modeSpeedDown: function () {
                return [0x31, 0x00, 0x00, 0x05, 0x02, 0x04, 0x00, 0x00, 0x00, zoneID]
            },
            command: function (fnName, arg, cb) {
                if (typeof arg === 'function') {
                    cb = arg;
    ...
    this.zoneCtlWhiteFactory = function (zoneID) {
        var zone = zoneID;
        if (zone > 4 || zone < 0) log.log('invalid zone');
        ...
        return {
            ...
            nightMode: function () {
                return [0x31, 0x00, 0x00, 0x01, 0x01, 0x06, 0x00, 0x00, 0x00, zoneID];
            },
            brightnessUp: function () {
                return [0x31, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00,  zoneID]
            },
            brightnessDown: function () {
                return [0x31, 0x00, 0x00, 0x01, 0x01, 0x02, 0x00, 0x00, 0x00, zoneID]
            },
            brightnessMax: function () {
                return [0x31, 0x00, 0x00, 0x01, 0x01, 0x06, 0x00, 0x00, 0x00, zoneID]
            },
            colorTempUp: function () {
                return [0x31, 0x00, 0x00, 0x01, 0x01, 0x03, 0x00, 0x00, 0x00, zoneID]
            },
            colorTempDown: function () {
                return [0x31, 0x00, 0x00, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, zoneID]
            },
            command: function (fnName, arg, cb) {
                if (typeof arg === 'function') {
                    cb = arg;
    ...
    this.baseCtlFactory = function () {
        ...
        return {
            ...
            brightnessSet: function (b) {
				if (b = 0) brightness = 174;
				if (b >  0 && b<=10) brightness = 187;
				if (b > 10 && b<=20) brightness = 199;
				if (b > 20 && b<=30) brightness = 228;
				if (b > 30 && b<=40) brightness = 246;
				if (b > 40 && b<=50) brightness = 254;
				if (b > 50 && b<=60) brightness = 29;
				if (b > 60 && b<=70) brightness = 49;
				if (b > 70 && b<=80) brightness = 63;
				if (b > 80 && b<=90) brightness = 80;
				if (b > 90) brightness = 106;
				brightness = Math.max(b, 0x00);
				brightness = Math.min(b, 0xFF);
				if (debug) log.log('Ich bin da Brightness' + brightness);
                return [0x31, 0x00, 0x00, 0x04, 0x01, brightness, brightness, brightness, brightness, zoneID];
            ...
            mode: function (mode) {
                return [0x31, 0x00, 0x00, 0x00, 0x04, mode, 0x00, 0x00, 0x00, zoneID];
            },
            command: function (fnName, arg, cb) {
                if (typeof arg === 'function') {
                    cb = arg;
    ...
    this.sendCmd = function (CMD, repeats, cb) {
        ...
        if (debug) log.log('chkSum ' + chkSum.toString('hex'));
        out = new Buffer(out);
        seqNum = (seqNum + 1) % 256;
    ...
    _func['8800000003'] = function (msg) {
        ...
        if (debug) log.log('1:'  + unknown1 ? unknown1.toString('hex') : 'unknown');
        if (debug) log.log('ID:' + bridgeID ? bridgeID.toString('hex') : 'unknown');
    };
